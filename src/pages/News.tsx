          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            {EVENTS.map((event, idx) => (
              <Reveal key={event.id} delay={idx * 0.1}>
                <div className="group rounded-[2rem] overflow-hidden border border-slate-200 dark:border-zinc-800 bg-white dark:bg-zinc-950 shadow-sm transition">
                  ...
                  <div className="p-6">
                    ...
                    <p className="mt-3 text-sm text-slate-500 dark:text-slate-400">
                     ...
                     <p className="mt-4 text-sm text-slate-600 dark:text-slate-300 leading-relaxed">
                      {truncateText(event.programOverview, 150)}
                    </p>
                    <div className="mt-6 flex flex-col gap-3 sm:flex-row">
                      <Button onClick={() => navigate(`/events/${event.id}`)} className="w-full sm:w-auto">
                        View In Detail
                      </Button>
